...
sont dans site-packages/optcgsim_tracker/) ET en développement (où les fichiers
sont à la racine du repo).

Usage :
    from optcgsim_tracker.resources import static_dir, has_frontend
    if has_frontend():
...
def static_dir() -> pathlib.Path | None:
    ...
    Ordre de résolution :
    1. optcgsim_tracker/static/ (dans le package — pip install)
    ...
    # 2. Racine du repo (mode dev) — remonte depuis optcgsim_tracker/ vers le repo.
    try:
